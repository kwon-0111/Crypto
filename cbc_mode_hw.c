...
int cbcEnc(U8 *key, const U8 *msg, U8 *cbc)
{
    ...
    for (i = 0; i < mb_len / BYTES; i++)
    {
        ...
        AES_encrypt(cxorm, cxorm, &enckey);
        memcpy(&cbc[(i + 1) * BYTES], cxorm, BYTES);
    }
    free(msg_block);
    return (i + 1) * BYTES;
}
int cbcDec(U8 *key, const U8 *cbc, int ct_len, U8 *dec_msg)
{
    ...
    for (i = 1; i < ct_len / 16; i++)
    {
        ...
        memcpy(cxorm, &cbc[i * BYTES], BYTES);
        AES_decrypt(cxorm, cxorm, &deckey); 
        // m_i = cxorm  xor  c_{i-1}
        for (j = 0; j < BYTES; j++)
        {
            cxorm[j] = cxorm[j] ^ cbc[(i - 1) * BYTES + j];
        }
        memcpy(dec_msg + (i - 1) * BYTES, cxorm, BYTES);
    }
